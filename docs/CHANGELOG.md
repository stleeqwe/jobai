...
## V6.1 - 크롤러 V2 + 통근시간 개선 (2026-01-16)

### 핵심 변경

| 항목 | 변경 내용 |
|------|----------|
| **크롤러** | V2 Lite 운영 검증 완료 (AJAX 기반) |
| **수집량** | 10,000건/회 (실측 검증) |
| **통근시간** | 항상 계산 (follow-up 필터 지원) |
| **키워드** | 제목 토큰 추출 → job_keywords 반영 |

### 크롤러 V2 성과

- **문제**: V1은 페이지네이션 미작동으로 ~700건만 수집
- **해결**: AJAX 엔드포인트 `/Recruit/Home/_GI_List` 발견
- **결과**: 10,000건 수집 (100% 성공률), 38분 소요

### API 제한 발견

- 잡코리아 AJAX API 페이지 제한 (시간대별 가변: 250-500페이지)
- **대안**: 구별 필터 크롤링으로 전체 64,000건 수집 가능
- 현재는 10,000건 테스트 모드 유지

### 미구현 (배포 시)

- 구별 전체 크롤링 (`run_crawl_by_gu.py`)
- Daily Sync 모드 (증분 업데이트)
- 스케줄러 설정 (systemd timer)
- 상세: `crawler/docs/CRAWLER.md`
...
| 날짜 | 버전 | 변경 내용 |
|------|------|----------|
| 2026-01-16 | V2.0 | **운영 검증 완료** (10,000건 성공) |
| 2026-01-16 | V2.0 | 제목 토큰 추출 → job_keywords 반영 |
| 2026-01-16 | V2.0 | 프록시 폴백 로직 검증 |
| 2026-01-15 | V2.0 | V2 Lite 초기 구현 (AJAX 기반) |
| 2026-01-15 | - | AJAX 엔드포인트 발견 |
| 2026-01-14 | V1 | `nearest_station` 필드 추가 |
| 2026-01-10 | V1 | MVP 크롤러 초기 구현 |

...
## 🚨 절대 규칙: 코드 수정 전 Claude 컨펌 필수

**어떤 코드 수정 요청이든 아래 순서를 반드시 따를 것.**
**이 규칙을 건너뛰면 안 됨. 특히 Step 5 (Claude 컨펌) 없이 코드 수정 절대 금지!**

```
협업 요청/직접 요청 수신 → [Step 1~2] 진단 & 기존 이슈 조회 → [Step 3] 이슈 생성
                        → [Step 4] 수정 내용 먼저 기록
                        → [Step 5] Claude에게 보고 & 컨펌 대기 ⬅️ 필수!
                        → [Step 6] 코드 수정 (컨펌 후)
                        → [Step 7] 검증 → [Step 8] 이슈 닫기
```

### ⚠️ 단독 코드 수정 금지 원칙

**Codex는 단독으로 코드를 수정하지 않습니다.** 반드시 아래 절차를 따라야 합니다:
...
이 원칙은 경로 A (직접 요청), 경로 B (협업 요청) 모두에 적용됩니다.
...
## Step 5: Claude에게 보고 및 컨펌 (필수!)

**코드 수정 전 반드시 Claude에게 분석 결과를 보고하고 컨펌을 받아야 합니다.**

### 보고 형식

```markdown
[Codex → Claude 수정 전 보고]
...
- Claude가 "승인" 또는 "진행해도 됨" 응답 시 → Step 6 진행
- Claude가 수정 요청 시 → 수정 사항 반영 후 재보고
- Claude가 거부 시 → 대안 검토 후 재보고

**⚠️ 컨펌 없이 Step 6으로 넘어가면 안 됨!**

---

## Step 6: 코드 수정 (컨펌 후)

Claude 컨펌을 받은 후에만 실제 코드 수정을 진행합니다.
...
1. **Claude 컨펌 없이 코드 수정 절대 금지** ⬅️ 최우선
2. **이슈 파일 없이 코드 수정 금지**
3. **기존 이슈 검색 없이 새 이슈 생성 금지**
4. **검증 없이 수정 완료 처리 금지**
5. **테스트 파일 루트에 생성 금지** → `tests/` 사용
6. **단독 판단으로 수정 범위 확대 금지** → Claude 협의 필요
...
1. **프록시 설정 확인**
   ...
   cat .env | grep PROXY_
   ```
   - `PROXY_HOST`, `PROXY_PORT`, `PROXY_USERNAME`, `PROXY_PASSWORD` 모두 필요
...
| 구성요소 | 예시 | 우선순위 |
|----------|------|----------|
| skills | Python, React, AWS | 1순위 (맨 앞) |
| work_fields | 백엔드개발, 마케팅 | 2순위 |
| title_tokens | 시니어, 주니어 | 3순위 |

**갯수 제한 없음** - 검색 정확도 향상을 위해 전체 포함
...
from app.db.firestore import get_db
db = get_db()
docs = db.collection('jobs').limit(5).stream()
for doc in docs:
    data = doc.to_dict()
    print(f\"{doc.id}: {data.get('job_keywords', [])[:5]}...\")
"
```

...
class SeoulSubwayCommute:
    ...
    서브클래스에서 오버라이드 가능한 설정:
    - BASE_COMMUTE_MINUTES: 기본 통근시간 (동일역 등)
    - GRID_SIZE: 공간 인덱싱 사용 여부 (0이면 비활성화)
    - format_time(): 시간 포맷팅 메서드
    """
    ...
    # 공간 인덱스 그리드 크기 (약 1km x 1km), 0이면 비활성화
    GRID_SIZE = 0.01  # 위도/경도 기준

    # 기본 통근시간 (서브클래스에서 오버라이드 가능)
    BASE_COMMUTE_MINUTES = 0
    ...
    def calculate_by_coords(
        ...
        if from_station == to_station:
            ...
            # BASE_COMMUTE_MINUTES 적용 (서브클래스에서 설정 가능)
            minutes = max(minutes, self.BASE_COMMUTE_MINUTES)
            rounded, text = self.format_time(minutes)
            return {
                'minutes': minutes,
                'text': text,
        ...
        # BASE_COMMUTE_MINUTES 적용
        total = max(total, self.BASE_COMMUTE_MINUTES)
        rounded, text = self.format_time(total)
    ...
    def format_time(self, minutes: int) -> Tuple[int, str]:
        """시간 포맷팅 (서브클래스에서 오버라이드 가능)

        기본 구현: 10분 단위 반올림, "약 N분" 형식
        Backend에서는 정확한 분 단위로 오버라이드 가능
        """
    ...
    # 하위 호환성을 위한 별칭
    _round_to_10_minutes = format_time
